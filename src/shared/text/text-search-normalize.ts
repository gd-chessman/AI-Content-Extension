/** Đổi ký tự “font kiểu” (Unicode math, fullwidth, …) về chữ Latin thường để so khớp. */
...
function demathOrStyledChar(cp: number): string | null {
  ...
  if (cp >= 0x1d56c && cp <= 0x1d585) return String.fromCharCode(0x41 + (cp - 0x1d585));
  ...
  if (cp >= 0x1d656 && cp <= 0x1d66f) return String.fromCharCode(0x41 + (cp - 0x1d656));
